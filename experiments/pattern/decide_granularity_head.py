...
## Hard Constraint
    ...
    str_R = 1
        ...
        対象プログラムの head_ast.code にマッチすること
        これを満たさない候補はスコープ選択の対象外とする
        目的：処理断片として元コード上に成立していることを確認する

## 出力

    outputs/pattern_HEAD/granularity_decided_all.json
        各 ID の粒度判定結果（選択スコープ・全候補スコア）

    outputs/pattern_HEAD/candidate_bone.json
...
def matches_source_code(regex: str, source_code: str) -> bool:
    ...
    Args:
        ...
        source_code: 対象プログラムの head_ast.code
...
def evaluate_str_r(
    ...
    Args:
        ...
        target_source_code: 対象プログラムの head_ast.code
...
def main() -> None:
    ...
    corpus_full_trees: dict[int, list[dict]] = {entry["id"]: entry["diff"]["head_ast"]["tree"] for entry in corpus_entries}
    # str_R 判定用のソースコード
    corpus_source_codes: dict[int, str] = {entry["id"]: entry["diff"]["head_ast"]["code"] for entry in corpus_entries}

    # ── スコープファイルの読み込み ──
    # 包含順（DIFF ⊆ BROTHER ⊆ EXCL ⊆ INCL）で定義することで
    # deduplicate_candidates が小さいスコープを優先して残す
    scope_file_specs: list[tuple[str, str]] = [
        ("diff", str(config.outputs / "AST_HEAD" / "scope_DIFF_BLOCK_all.json")),
        ("brother", str(config.outputs / "AST_HEAD" / "scope_BROTHER_DIFF_all.json")),
        ("excl", str(config.outputs / "AST_HEAD" / "scope_BLOCK_EXCLUDE_PARENT_all.json")),
        ("incl", str(config.outputs / "AST_HEAD" / "scope_BLOCK_INCLUDE_DIFF_all.json")),
    ]
    # scope_file_specs: list[tuple[str, str]] = [
    #     ("diff", str(config.outputs / "AST_HEAD" / "scope_DIFF_BLOCK_targets.json")),
    #     ("brother", str(config.outputs / "AST_HEAD" / "scope_BROTHER_DIFF_targets.json")),
    #     ("excl", str(config.outputs / "AST_HEAD" / "scope_BLOCK_EXCLUDE_PARENT_targets.json")),
    #     ("incl", str(config.outputs / "AST_HEAD" / "scope_BLOCK_INCLUDE_DIFF_targets.json")),
    # ]
    ...
    # ── 結果の保存 ──
    # 全データ版fast
    output_path = config.outputs / "pattern_HEAD" / "granularity_decided_all.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    hayalab.write_json(str(output_path), output_records)
    print(f"wrote {len(output_records)} entries -> {output_path}")

    bone_path = config.outputs / "pattern_HEAD" / "instruction_granularity_decided_all.json"
    hayalab.write_json(str(bone_path), instruction_records)
    print(f"wrote {len(instruction_records)} entries -> {bone_path}")

    # ターゲットデータ版 fast
    # output_path = config.outputs / "pattern_HEAD" / "granularity_decided_targets.json"
    # output_path.parent.mkdir(parents=True, exist_ok=True)
    # hayalab.write_json(str(output_path), output_records)
    # print(f"wrote {len(output_records)} entries -> {output_path}")

    # bone_path = config.outputs / "pattern_HEAD" / "instruction_granularity_decided_targets.json"
    # hayalab.write_json(str(bone_path), instruction_records)
    # print(f"wrote {len(instruction_records)} entries -> {bone_path}")
